...
# 27th May 2017: tidied up script
  #-------------------------------------------------------------------------------
  # Specify packages
  #-------------------------------------------------------------------------------
 ...
 xlsdir<-"~/Dropbox/Projects2016/SQING/excel sheets/xls_for_osf/"
  myfilename<-paste(xlsdir,"SQING_with_templates.xlsx",sep='') 
  
  SQING.dat=loadWorkbook(myfilename)
 #-------------------------------------------------------------------------------
 
# The organisation of information in the xls file is not optimal so we will make
#  a new data frame with column headings that make sense and then copy over
...
temp5=data.frame(readWorksheet(SQING.dat,sheet='authorcontact',startCol=4,endCol=4,startRow=2,endRow=31,header=F))
 allstudy[,48]<-temp5[,1]
 print('Ns who replied (2), acknowledged (1), no response (0)')
...
# Add journal names from sheet 'studies_with_replicates'
...
myNtable$Corrected<-c(1,1,0,0,1,0,1,0,1,1,0,0,0,1,2,2,2,2,0,0,2,1,0,0,2,2,0,1,0,2)
...
lines(myline,lty=2,lwd=2,col='red') #plot dotted line showing effect size with 80% power

#compute correlation of effect size with log sample size
...
#redo correlation excluding those with quasi effect sizes
